...
# All rights reserved.
 ...
 dgaitzeta <-
  ...
  LLL <- max(length(shape.p),   length(x),
             length(pobs.mix.a), length(shape.a),
             length(pstr.mix.i), length(shape.i))  # added
  ...
  lhs.prob <- pzeta(max.support, shape.p)  # Usually 1
  ...
  if (linfla.mlm) {
    if (deflation) {
      ...
      for (jay in 1:linfla.mlm) {
        ...
        pmf.i <- dzeta(inflate.mlm[jay], shape.p[vecTF])
  ...
  pmf0[!skip] <-
    (tmp6 * dzeta(x, shape.p) / (lhs.prob - suma - sumt))[!skip]
 ...
 pgaitzeta <-
  ...
  if (lalter.mix + lalter.mlm + linfla.mix + linfla.mlm + ltrunc == 0 &&
      is.infinite(max.support))
    return(pzeta(q, shape.p))  # lower.tail, log.p
  ...
  LLL <- max(length(shape.p),   length(q),
             length(pobs.mix.a), length(shape.a),
             length(pstr.mix.i), length(shape.i))  # added
  ...
  if (linfla.mix) {
    ...
    for (jay in seq(linfla.mix)) {
      ...
      use.pstr.mix.i[, jay] <- dzeta(ival, shape.i)
    }
 ...
 qgaitzeta <-
  ...
  LLL <- max(length(shape.p),   length(p),
             length(pobs.mix.a), length(shape.a),
             length(pstr.mix.i), length(shape.i))  # added
 ...
 dgaitlog <-
  ...
  vecTF.t <- is.finite(x) & ((x %in% truncate) | (max.support < x))
  lhs.prob <- plog(max.support, shape.p)  # Usually 1
  denom.t <- lhs.prob - sumt  # No sumt on RHS
  ...
  if (linfla.mlm) {
    if (deflation) {
      ...
      for (jay in 1:linfla.mlm) {
        vecTF <- is.finite(x) & inflate.mlm[jay] == x
        pmf.i <- dlog(inflate.mlm[jay], shape.p[vecTF])
        if (any(pstr.mlm.i[vecTF, jay] <
                -(tmp6[vecTF] + pstr.mlm.i[vecTF, jay]) * pmf.i / (
                  tmp0[vecTF] - pmf.i), na.rm = TRUE)) {
          warning("too much deflation in argument 'pstr.mlm.i'. ",
  ...
  pmf0[!skip] <- (tmp6 *
    dlog(x, shape.p) / (lhs.prob - suma - sumt))[!skip]  # added
 ...
 pgaitlog <-
  ...
  if (lalter.mix + lalter.mlm + linfla.mix + linfla.mlm + ltrunc == 0 &&
      is.infinite(max.support))
    return(plog(q, shape.p))  # lower.tail, log.p
  ...
  if (linfla.mix) {
    ...
    for (jay in seq(linfla.mix)) {
      ival <- inflate.mix[jay]
      use.pstr.mix.i[, jay] <- dlog(ival, shape.i)
    }
    use.pstr.mix.i <- pstr.mix.i *
                      use.pstr.mix.i / rowSums(use.pstr.mix.i)
