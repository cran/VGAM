...
uqo.control <-
  function(Rank = 1,
          ...
          Hstep = 0.001,
          isd.latvar = rep(c(2, 1, rep(0.5, len = Rank)), len = Rank),
          ITolerances = FALSE,
          lvstart = NULL,
  ...
  if (any(isd.latvar < 0.2 | isd.latvar > 10))
...
calluqof <- function(sitescores, etamat, ymat, wvec, modelno,
                     nice31, xmat, Control,
    ...
    pstar <- othint[3]
    nstar <- if (nice31) ifelse(modelno==3 || modelno==5,
                                n*2, n) else n*M
  ...
  ans1 <- if (nice31) .Fortran("cqo1f",
     ...
     if (ans1$errcode == 0) {
        ...
        if (alldump) {
          ans1$fv <- matrix(ans1$fv, n, M, byrow = TRUE,
...
callduqof <-
  function(sitescores, etamat, ymat, wvec, modelno, nice31, xmat,
           Control,
    ...
    ans1 <- 
    .Fortran("duqof", numat = as.double(numat), as.double(ymat),
           as.double(xmat),
           as.double(wvec), etamat = as.double(usethiseta),
           moff = double(if(itol) n else 1),
           fv = double(NOS*n), z = double(n*M), wz = double(n*M),
...
uqo.fit <- function(x, y, w = rep(1, len = nrow(x)),
    ...
    sitescores <- if (length(rrcontrol$lvstart)) {
      ...
      if (rrcontrol$Use.Init.Poisson) {
        .Init.Poisson.QO(ymat = as.matrix(y),
          ...
          matrix((runif(n*Rank) - 0.5) * rrcontrol$sd.sitescores,
                 n, Rank)
        }
    }
    ...
    othint <- c(Rank, control$EqualTol, pstar,
                dimw = 1, inited=290, # other ints
                modelno, maxitl=control$maxitl, actnits = 0,
                twice = 0, p1star,
                p2star, nice31, lenbeta, control$ITolerances,
                control$trace,
  ...
  for (iter in 1:optim.maxit) {
    ...
    if (nice31) {
      ...
      if (rrcontrol$IToleran) {
        ...
        B1 <- coefs[, (1+(Rank + Rank*(Rank+1)/2)):ncol(coefs),
                      drop = FALSE]
      }
    } else {
    ...
    latvar.names <- if (Rank == 1) "latvar" else
                                   paste("latvar", 1:Rank, sep = "")
    lp.names <- predictors.names
    if (!length(lp.names)) lp.names <- NULL
    extra$Amat <- matrix(Amat, M, Rank,
                         dimnames = list(lp.names, latvar.names))
...
show.uqo <- function(object) {
  ...
  cat("\n")
  cat(object@misc$n, "sites and",
      object@misc$M, "responses/species\n")
...
setMethod("lvplot", "uqo", function(object, ...) 
         invisible(lvplot.qrrvglm(object, C = FALSE,
                                  check.ok = FALSE, ...)))
...
if (FALSE) {
  scores.uqo <- function (x, type = c("sites", "species"), ...) {
    ...
    switch(type,
           sites = if (any(slotNames(x) == "latvar"))
                     x@latvar else
                     Coef(x)@latvar,
           species = if (any(slotNames(x) == "Optimum"))
                       x@Optimum else
                       Coef(x)@Optimum)
  }
...
setMethod("latvar", "uqo", function(object, ...)
...
setMethod("Tol", "Coef.uqo",
          function(object, ...) Tol.Coef.uqo(object, ...))
