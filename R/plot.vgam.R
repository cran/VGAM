...
vplot.numeric <-
  ...
  if (overlay) {
    ...
    if (rugplot)
      rug(jx, col = rcol)
    if (se && !is.null(se.y)) {
      matlines(ux, se.upper[, which.cf], lty =  slty, lwd = slwd,
               col = scol)
      matlines(ux, se.lower[, which.cf], lty =  slty, lwd = slwd,
               col = scol)
    }
  } else {
    YLAB <- ylab
...
vplot.factor <-
  ...
  if (!is.null(residuals))  {
    ...
    if (overlay) {
warning("overlay = TRUE in vplot.factor: assigning overlay <- FALSE")
      vvplot.factor(x, y,
